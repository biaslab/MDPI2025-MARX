function [u U uonce lines randlines] = fMultiSinGen()

[Ntest, Ptest, Mtest, fMinTest, fMaxTest, fsTest] = readN()
options.N = Ntest;
options.P = Ptest;
options.M = Mtest;
options.fMin = fMinTest;
options.fMax = fMaxTest;
options.fs = fsTest;
options.type =  'odd';
% 
...
nLines = length(lines);
fprintf('matlab: %d %d %d\n', nLines, N, M);

%% multisine generation - frequency domain implementation
U = zeros(N,M);
randlines = rand(nLines,M);
U(lines,:) = exp(2i*pi*randlines); % excite the selected frequencies
